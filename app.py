"""
StyleSync - Wardrobe & Outfit Matching (no AI, no avatar)

Run:
    pip install -r requirements.txt
    streamlit run app.py

This version removes the AI virtual try-on / avatar ("doll") entirely.
Outfits are shown as a clean flat-lay of your real garment photos, each
with a rule-based Match Score. Wardrobes, suggestions, and stats are kept
separate for Male and Female (a "Unisex" tag makes an item eligible for
both). Everything here is deterministic Python - no model calls at all.
"""
...
st.markdown(
    ...
    <div class="hero">
        ...
        <span class="badge">♡ Gender-separated wardrobes · Match Score · Favorites</span>
    </div>
    """,
    unsafe_allow_html=True,
)
...
with tab_dash:
    ...
    for dash_tab, gender in [(dash_male, "Male"), (dash_female, "Female")]:
        with dash_tab:
            total, week, last, avg_score = db.get_stats(gender)
            ...
            history = db.get_wear_history(gender, weeks=12)
    ...
    with st.form("add_item_form", clear_on_submit=True):
        ...
        if submitted:
            if not name.strip():
                ...
                db.add_item(type_, name.strip(), color, occasion, gender, str(image_path))
                st.success(f"Added **{name}** to the {gender} wardrobe. ✨")
                st.rerun()
...
with tab_suggest:
    ...
    st.markdown(
        ...
        <div class="info-card">
            ...
            <b>real uploaded photos</b> that pass our color-compatibility rules, then
    ...
    if st.button("✨ Get My Look", type="primary"):
        tops = [i for i in db.get_items("top", gender_pick) if i["occasion"] == occasion_filter]
        bottoms = [i for i in db.get_items("bottom", gender_pick) if i["occasion"] == occasion_filter]
        shoes = [i for i in db.get_items("shoes", gender_pick) if i["occasion"] == occasion_filter]
        ...
        if not (tops and bottoms and shoes):
            st.warning(
                f"Add at least one photo-based top, bottom and shoe for the "
            ...
            if not valid:
                ...
                recent = db.get_recent_outfit_ids(gender_pick)
    ...
    if st.session_state.get("outfit") and st.session_state.get("outfit_gender") == gender_pick:
        ...
        with b1:
            if st.button("✓ Wear This Today", type="primary"):
                ...
                db.log_outfit(t, b, s, gender_pick, score)
        ...
        with b2:
            if st.button("♡ Save to Favorites"):
                t, b, s = outfit_ids(st.session_state["outfit"])
                db.add_favorite(t, b, s, gender_pick, score)
                st.success("Saved to Favorites! 💖")
        with b3:
            if st.button("🔄 Try Another"):
                recent = db.get_recent_outfit_ids(gender_pick)
                tops = [i for i in db.get_items("top", gender_pick) if i["occasion"] == occasion_filter and i.get("image_path") and os.path.exists(i["image_path"])]
                bottoms = [i for i in db.get_items("bottom", gender_pick) if i["occasion"] == occasion_filter and i.get("image_path") and os.path.exists(i["image_path"])]
                shoes = [i for i in db.get_items("shoes", gender_pick) if i["occasion"] == occasion_filter and i.get("image_path") and os.path.exists(i["image_path"])]
                valid = logic.generate_valid_outfits(tops, bottoms, shoes)
                if valid:
                    chosen = logic.pick_outfit(valid, recent)
                    st.session_state["outfit"] = chosen
...
with tab_fav:
    ...
    favorites = db.get_favorites(fav_gender)

    if not favorites:
        st.info("No favorites saved yet — save a look from **Style Me**.")
    else:
        for fav in favorites:
            top = db.get_item_by_id(fav["top_id"])
            bottom = db.get_item_by_id(fav["bottom_id"])
            shoe = db.get_item_by_id(fav["shoes_id"])
            ...
            with fb1:
                if st.button("✓ Wear This Today", key=f"wear_fav_{fav['id']}", type="primary"):
                    db.log_outfit(top["id"], bottom["id"], shoe["id"], fav_gender, fav["match_score"])
                    st.success("Look logged! ✨")
                    st.rerun()
            with fb2:
                if st.button("🗑 Remove", key=f"del_fav_{fav['id']}"):
                    db.delete_favorite(fav["id"])
                    st.rerun()
